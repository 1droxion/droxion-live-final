...
export default function DroxionChat() {
  ...
  async function openThread(person) {
    ...
    if (user?.id && person?.user_id) {
      await supabase.from('droxion_direct_messages').update({ read_at: new Date().toISOString() }).eq('recipient_id', user.id).eq('sender_id', person.user_id).is('read_at', null);
      await load();
    }
  }

  function closeThread() { setActive(null); setDraft(''); setNotice(''); }
