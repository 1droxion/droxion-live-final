...
async function createMixedAudioTrack(micStream, screenStream) {
  ...
  const addTrack = (track, gainValue) => {
...
export async function createLiveStudioStream({
  ...
  let disposed = false;
  let currentLayout = layout || defaultStudioLayout(orientation);
  ...
  try {
    ...
    const displaySurface = String(screenTrack.getSettings?.().displaySurface || 'unknown');
    ...
    const outputTracks = [screenTrack];
    ...
    if (mode === LIVE_SOURCE_MODE.SCREEN_CAMERA) {
      ...
      outputTracks.push(cameraTrack);
    }

    try { mixedAudio.track.__droxionSource = 'audio'; } catch {}
    outputTracks.push(mixedAudio.track);
    outputStream = new MediaStream(outputTracks);
    try {
      outputStream.__droxionStudio = {
        mode,
        orientation,
        displaySurface
      };
    ...
    const controller = {
      ...
      setLayout: nextLayout => {
        ...
        currentLayout = allowed.some(item => item.id === nextLayout) ? nextLayout : defaultStudioLayout(orientation);
