...
function getLiveEventStream(sessionId) {
  ...
  const channel = client
    .channel(`droxion-live-events:${sessionId}`)
    .on(
      "postgres_changes",
      ...
      payload => {
        ...
        if (row.event_type === "chat") {
          ...
          // Wake the next reconciliation read only when guest state actually changes.
    ...
    .subscribe(status => {
      if (status === "SUBSCRIBED") stream.ready = true;
    });
...
function cleanupOldLiveStreams() {
  ...
  for (const [sessionId, stream] of liveEventStreams.entries()) {
    ...
    try { client.removeChannel(stream.channel); } catch {}
