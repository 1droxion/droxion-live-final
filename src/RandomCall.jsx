...
function iceServers() {
  ...
  if (turnUrl) {
    servers.push({ urls: turnUrl, username: import.meta.env.VITE_TURN_USERNAME || '', credential: import.meta.env.VITE_TURN_CREDENTIAL || '' });
  }
  return servers;
}
...
export default function RandomCall() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!authUser) { setPhase('guest'); return; }
      const { data: bonus } = await supabase.rpc('droxion_claim_welcome_bonus');
      if (bonus?.coin_balance != null) setCoins(Number(bonus.coin_balance));
      else {
  ...
  const endCall = useCallback(async (goBack = true) => {
    ...
    setCallId(null); setPartner(null); setSeconds(0);
    connectedOnce.current = false; lastSignalId.current = 0;
  ...
  async function beginSearch() {
    if (!user) return;
    setMessage('');
    if (filter === 'female' && Number(coins || 0) < 3) { setMessage('You need at least 3 coins to use the Female filter.'); return; }
    setPhase('searching');
    const { data, error } = await supabase.rpc('droxion_join_random_queue', { p_filter: filter });
    if (error) { setMessage(error.message || 'Could not start matching.'); setPhase('choose'); return; }
    ...
    (async () => {
      try {
        ...
        pc.ontrack = async event => {
          ...
          if (!connectedOnce.current) {
            ...
            if (error || !data?.allowed) {
              ...
              await endCall(false); setPhase('choose'); return;
            }
        ...
        pc.onicecandidate = async event => {
          ...
          const status = await supabase.rpc('droxion_random_status');
          const recipient = status.data?.partner_id;
          if (!recipient) return;
          await supabase.from('droxion_call_signals').insert({ call_id: callId, sender_id: user.id, recipient_id: recipient, signal_type: 'ice', payload: event.candidate.toJSON() });
        };
        if (isInitiator) {
          const offer = await pc.createOffer();
          await pc.setLocalDescription(offer);
          const status = await supabase.rpc('droxion_random_status');
          await supabase.from('droxion_call_signals').insert({ call_id: callId, sender_id: user.id, recipient_id: status.data?.partner_id, signal_type: 'offer', payload: offer });
        }
      } catch (err) {
        setMessage('Camera and microphone permission are required for a video call.');
        await endCall(false); setPhase('choose');
      }
    })();
    return () => { stopped = true; };
  }, [phase, callId, user?.id, isInitiator, endCall]);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      ...
      for (const row of rows || []) {
        ...
        try {
          if (row.signal_type === 'offer' && !isInitiator) {
            ...
            const status = await supabase.rpc('droxion_random_status');
            await supabase.from('droxion_call_signals').insert({ call_id: callId, sender_id: user.id, recipient_id: status.data?.partner_id, signal_type: 'answer', payload: answer });
          } else if (row.signal_type === 'answer' && isInitiator && !pc.currentRemoteDescription) {
            await pc.setRemoteDescription(new RTCSessionDescription(row.payload));
      ...
      if (status?.status === 'ended') { setMessage('The call ended.'); await endCall(false); setPhase('choose'); }
    }, 900);
    return () => clearInterval(timer);
  }, [callId, user?.id, phase, isInitiator, endCall]);
  ...
  useEffect(() => {
    ...
    billingTimer.current = setInterval(async () => {
      ...
      if (data?.coin_balance != null) setCoins(Number(data.coin_balance));
      if (error || !data?.allowed) { setMessage('Call ended because there are not enough coins.'); await endCall(false); setPhase('choose'); }
    }, 10000);
    return () => { clearInterval(clock); if (billingTimer.current) clearInterval(billingTimer.current); billingTimer.current = null; };
  }, [phase, callId, endCall]);

  function toggleMic() { const t = streamRef.current?.getAudioTracks()?.[0]; if (!t) return; t.enabled = !t.enabled; setMicOn(t.enabled); }
  function toggleCamera() { const t = streamRef.current?.getVideoTracks()?.[0]; if (!t) return; t.enabled = !t.enabled; setCameraOn(t.enabled); }
  async function switchCamera() {
    const current = streamRef.current?.getVideoTracks()?.[0]; if (!current) return;
    const nextMode = current.getSettings().facingMode === 'user' ? 'environment' : 'user';
    const newStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: nextMode }, audio: false });
    const newTrack = newStream.getVideoTracks()[0];
    const sender = pcRef.current?.getSenders().find(s => s.track?.kind === 'video');
    await sender?.replaceTrack(newTrack); current.stop();
