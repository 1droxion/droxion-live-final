...
const LIVE_DISCOVERY_LIMIT = 260;
...
export default function GlobalLiveHub({ query = '', nativeLive = null, currentUserId, coins = 0, onCoinsChanged, onOpenWallet, mode = 'home' }) {
  ...
  const loadStreams = useCallback(async ({ manual = false } = {}) => {
    ...
    try {
      ...
      setStreams((Array.isArray(data?.streams) ? data.streams : []).filter(stream => ['youtube', 'kick', 'twitch', 'tango', 'liveme', 'poppo'].includes(stream?.provider)));
  ...
  return <section className={`dxGlobalLiveHub dxHubMode-${mode}`}>
    {mode === 'home' ? <div className="dxGlobalHero"><div className="dxHeroCopy"><span className="dxGlobalEyebrow"><Globe2 size={15} /> DROXION LIVE NETWORK</span><h1>Live everywhere.<br /><em>Picked for you.</em></h1><p>Discover YouTube, Kick, Twitch and partner LIVE streams in one personalized home, then watch, chat and support from Droxion.</p><div className="dxHeroProof"><span><Sparkles size={14} /> Personalized</span><span><ShieldCheck size={14} /> Watch inside Droxion</span><span><Radio size={14} /> {totalAvailable || '—'} LIVE now</span></div></div><button type="button" className="dxGlobalRefresh" onClick={() => loadStreams({ manual: true })} disabled={refreshing}><RefreshCw size={17} className={refreshing ? 'spin' : ''} /><span>{refreshing ? 'Refreshing' : 'Refresh LIVE'}</span></button></div> : <div className="dxExploreHeader"><div><span><Compass size={15} /> {pageKicker}</span><h1>{pageTitle}</h1><p>Your followed creators across YouTube, Kick, Twitch and partner LIVE sources, together in one place.</p></div><button type="button" onClick={() => loadStreams({ manual: true })}><RefreshCw size={16} className={refreshing ? 'spin' : ''} /> Refresh</button></div>}
