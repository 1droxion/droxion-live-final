...
const EXTERNAL_PROVIDERS = [
  ...
  { id: 'liveme', label: 'LiveMe' },
  { id: 'poppo', label: 'Poppo' }
];
...
const LIVE_DISCOVERY_LIMIT = 200;
...
function providerClass(provider) {
  ...
  if (provider === 'trovo') return 'trovo';
  if (provider === 'tango') return 'tango';
  if (provider === 'liveme') return 'liveme';
  if (provider === 'poppo') return 'poppo';
  return 'droxion';
}
...
const isIOSNative =
  ...
  else if (stream?.embedType === 'trovo' && stream.channelSlug) src = `https://player.trovo.live/embed/player?streamername=${encodeURIComponent(stream.channelSlug)}&muted=0&fullscreen=1&hidecontrol=0&autoplay=1&hidefollow=0&hidesub=0&sdist=droxion`;
  else if (['tango', 'liveme', 'poppo'].includes(stream?.embedType) && stream.embedUrl) src = stream.embedUrl;
...
export default function GlobalLiveHub({ query = '', nativeLive = null, currentUserId, coins = 0, onCoinsChanged, onOpenWallet, mode = 'home' }) {
  ...
  const loadStreams = useCallback(async ({ manual = false } = {}) => {
    ...
    try {
      ...
      setStreams((Array.isArray(data?.streams) ? data.streams : []).filter(stream => ['youtube', 'kick', 'twitch', 'rumble', 'trovo', 'tango', 'liveme', 'poppo'].includes(stream?.provider)));
  ...
  return <section className={`dxGlobalLiveHub dxHubMode-${mode}`}>
    {mode === 'home' ? <div className="dxGlobalHero"><div className="dxHeroCopy"><span className="dxGlobalEyebrow"><Globe2 size={15} /> DROXION LIVE NETWORK</span><h1>Live everywhere.<br /><em>Picked for you.</em></h1><p>Discover YouTube, Kick, Twitch, Rumble, Trovo and partner LIVE streams in one personalized home, then watch, chat and support from Droxion.</p><div className="dxHeroProof"><span><Sparkles size={14} /> Personalized</span><span><ShieldCheck size={14} /> Watch inside Droxion</span><span><Radio size={14} /> {totalAvailable || '—'} LIVE now</span></div></div><button type="button" className="dxGlobalRefresh" onClick={() => loadStreams({ manual: true })} disabled={refreshing}><RefreshCw size={17} className={refreshing ? 'spin' : ''} /><span>{refreshing ? 'Refreshing' : 'Refresh LIVE'}</span></button></div> : <div className="dxExploreHeader"><div><span><Compass size={15} /> {pageKicker}</span><h1>{pageTitle}</h1><p>Your followed creators across YouTube, Kick, Twitch, Rumble, Trovo and partner LIVE sources, together in one place.</p></div><button type="button" onClick={() => loadStreams({ manual: true })}><RefreshCw size={16} className={refreshing ? 'spin' : ''} /> Refresh</button></div>}
