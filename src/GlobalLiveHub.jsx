    ...
    <section className="dxGlobalLiveHub">
      ...
      <div className="dxGlobalProviderRail" aria-label="LIVE sources">
        {PROVIDERS.map(item => {
          ...
          return <button type="button" key={item.id} className={`${active ? 'active' : ''} ${enabled ? '' : 'disabled'}`} onClick={() => setProvider(item.id)} disabled={!enabled && external}><span>{item.label}</span>{external && Number(providers?.[item.id]?.available || 0) > 0 && <small>{providers[item.id].available}</small>}</button>;
        })}
      </div>
