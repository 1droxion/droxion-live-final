...
const LIVE_HUB_CACHE_KEY = 'droxion.live.hub.cache.v3';
...
function readLiveHubCache() {
  ...
  try {
    ...
    return {
      streams: cached.streams.filter(stream => ['youtube', 'kick', 'twitch', 'rumble'].includes(stream?.provider)).slice(0, LIVE_DISCOVERY_LIMIT),
...
export default function GlobalLiveHub({ query = '', nativeLive = null, currentUserId, coins = 0, onCoinsChanged, onOpenWallet, mode = 'home' }) {
  ...
  const loadStreams = useCallback(async ({ manual = false } = {}) => {
    ...
    const allowed = ['youtube', 'kick', 'twitch', 'rumble'];
    let lastError = null;
