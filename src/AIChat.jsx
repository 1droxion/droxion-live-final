...
function AIChat() {
    ...
    try {
      if (prompt.toLowerCase().includes("image")) {
        ...
        setMessages((prev) => [
          ...
          {
            role: "assistant",
            content: `<b>📺 ${title}</b><br/><iframe width="100%" height="215" src="https://www.youtube.com/embed/${videoId}" frameborder="0" allowfullscreen></iframe>`,
          },
        ]);
      } else {
  ...
  return (
    <>
      <style>{`html, body { max-width: 100%; overflow-x: hidden; touch-action: manipulation; }`}</style>
      <div className="bg-black text-white flex flex-col min-h-screen h-[100dvh] overflow-hidden">
        <div className="text-center pt-4 pb-2">
          <h1 className="text-2xl font-bold text-gray-400 tracking-widest">Droxion</h1>
        </div>

        <div className="flex-1 overflow-y-auto px-3 max-w-3xl mx-auto w-full pb-36">
          {messages.length === 0 ? (
