...
function AIChat() {
  ...
  useEffect(() => {
    if (!localStorage.getItem("user_id")) {
      localStorage.setItem("user_id", "user_" + Math.random().toString(36).substring(2, 10));
    }
  }, []);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, {
        role: "assistant",
        content: "⚠️ AI Error or connection failed. Try again."
      }]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className={`flex flex-col min-h-screen ${darkMode ? "bg-black text-white" : "bg-white text-black"}`}>
      <div className="text-center pt-4 pb-2">
        <h1 className="text-2xl font-bold tracking-widest text-gray-400">Droxion AGI</h1>
      </div>
      ...
      {!started && (
        <div className="absolute inset-0 flex flex-col items-center justify-center px-4">
          <div className="max-w-xl w-full bg-[#111] rounded-2xl p-4 border border-gray-800">
            ...
            <div className="flex items-center bg-[#000] rounded-full px-4 py-3">
              <input
                type="text"
            ...
            <p className="text-center text-gray-500 text-xs mt-4">
              Try: "Generate image of Mars" or "Latest news"
            </p>
          </div>
        </div>
      )}
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-black border-t border-gray-800 px-3 py-3 z-50">
          <div className="flex justify-between items-center max-w-3xl mx-auto mb-2 px-1">
            ...
            <div className="flex gap-3 items-center">
              <FaTrash onClick={() => { setMessages([]); setStarted(false); }} className="text-white text-lg cursor-pointer" />
              <button onClick={() => setDarkMode(!darkMode)} className="text-white text-xl">
                {darkMode ? <FaSun /> : <FaMoon />}
              </button>
            </div>
