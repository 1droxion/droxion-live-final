...
function AIChat() {
  ...
  const SmartCard = ({ card }) => {
      ...
      return (
        <div className="glass rounded-lg p-3">
          <div className="flex items-center gap-3">
            {card.icon && (
              <img src={card.icon} alt="" width={48} height={48} referrerPolicy="no-referrer" />
            )}
            <div>
    ...
    // Generic web/wiki/news/stock/weather link
  ...
  const handleSend = async (textToSend = input) => {
    ...
    try {
      ...
      if (ytKW.some((k) => lower.includes(k)) || lower.startsWith("youtube:")) {
        ...
        if (directId) {
          ...
          try {
            ...
            else pushAssistant("I couldn't find a video for that.");
          } catch {
      ...
      if (imageTrigger) {
        ...
        try {
          const rr = await axios.post(`${API_BASE}/realtime`, {
            query: q || content,
            intent: "images"
          });
          const cards = Array.isArray(rr.data?.cards) ? rr.data.cards.filter(Boolean) : [];
          if (cards.length) {
            pushAssistant(`### Images for **${q || "your query"}**`, { cards });
          } else {
            const imgUrl = `https://www.google.com/search?tbm=isch&q=${encodeURIComponent(q || content)}`;
            pushAssistant(`### Images for **${q || "your query"}**`, {
              cards: [{ type: "web", title: `Google Images – ${q || "search"}`, url: imgUrl, source: "google.com", snippet: "Open to view high-res images." }]
            });
          }
        } catch {
          const imgUrl = `https://www.google.com/search?tbm=isch&q=${encodeURIComponent(q || content)}`;
      ...
      {suggestions.length > 0 && (
        ...
        >
          <div className="max-w-4xl mx-auto px-3">
            <div
              className="glass rounded-xl p-2 pointer-events-auto"
