// src/AIChat.jsx — Droxion (Image Edit MVP)
// - FIX: understands bg-swap shape {subject_png:[], background:[]}
// - Shows cutout + background + a quick composite preview
// - Robust to {images:[]} | {outputs:[]} | plain array
// - Simple mask painter for inpaint
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      text:
        "Upload a photo, write a prompt, pick a mode, then Create. " +
        "For Background Swap, the app returns a subject cutout and a generated background."
    }
  ]);
  ...
  function Composite({ subjectUrl, backgroundUrl }) {
    ...
    useEffect(() => {
      ...
      bg.onload = () => {
        ...
        fg.onload = () => {
          // draw subject at ~60% height, scale to 55% of width
      ...
      <div className="chat">
        {messages.map((m, idx) => (
          <div key={idx} className={`msg ${m.role}`}>
            ...
            {/* bg-swap composite preview if both pieces exist */}
