// Droxion — phone-first UI (icons aligned, proper theme, clean bottom bar)
...
import {
  ...
  FiFile, FiGlobe, FiCpu, FiBook, FiSearch, FiAperture, FiTrash2, FiRefreshCcw
} from "react-icons/fi";
import "./AIChat.css";

const API_BASE = "https://droxion-backend.onrender.com";

/* ---------------- helpers / (most of yours kept intact) ---------------- */
...
const faviconFor = (u="") => { const h = host(u); if (!h) return null; return `https://www.google.com/s2/favicons?sz=64&domain=${encodeURIComponent(h)}`; };
...
const wantsWeather = (s="") => { const q=s.trim().toLowerCase(); if (/\b(weather|temp|temperature|forecast|rain|humidity|wind)\b/.test(q)) return true; if (/\bwether\b|\bwheather\b/.test(q)) return true; if (/^(whether|wether)$/i.test(q)) return true; return false; };
const wantsCrypto  = (s="") => /\b(crypto|bitcoin|btc|ethereum|eth|price|chart)\b/i.test(s);
const isSearchy = (s="") => { const q=s.toLowerCase(); return ( q.startsWith("google:") || q.startsWith("search:") || /\b(now|today|latest|breaking|live|update|news)\b/.test(q) || /\b(price|stock|chart|net worth|time|weather|forecast|crypto|btc|eth)\b/.test(q) || wantsNews(q) || wantsWeather(q) || wantsCrypto(q) ); };
...
const HQ = ["forbes.com","bloomberg.com","reuters.com","cnbc.com","apnews.com","ft.com","wsj.com","nytimes.com","theguardian.com","bbc.com","bbc.co.uk","npr.org","hindustantimes.com","livemint.com","moneycontrol.com","economictimes.com","coindesk.com","cointelegraph.com","coinmarketcap.com","coingecko.com","messari.io","defillama.com","finance.yahoo.com","google.com","tradingview.com","marketwatch.com","morningstar.com","nasdaq.com","seekingalpha.com","sec.gov","weather.com","accuweather.com","time.is","timeanddate.com","techcrunch.com","theverge.com","wired.com","arstechnica.com","wikipedia.org","medium.com"];
...
function ToolsMenu({
  ...
  onSetPersona,
  onCreateImage,
  onToggleWebSearch, webSearchOn,
  onClearAll, onNewChat
}) {
  ...
  return (
    <div className="menu-panel">
      <input ref={camRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={(e)=>{ const f=e.target.files?.[0]; if(f) onSendImageFile(f,{source:"camera"}); e.target.value=""; }}/>
      <input ref={photosRef} type="file" accept="image/*" className="hidden" onChange={(e)=>{ const f=e.target.files?.[0]; if(f) onSendImageFile(f,{source:"photos"}); e.target.value=""; }}/>
      <input ref={filesRef} type="file" className="hidden" onChange={(e)=>{ const f=e.target.files?.[0]; if(f) onSendAnyFile?.(f); e.target.value=""; }}/>
      ...
      {/* moved here per your request */}
      <button className="menu-item" onClick={onClearAll}><FiTrash2 className="icon"/><span>Clear Chat + Memory</span></button>
      <button className="menu-item" onClick={onNewChat}><FiRefreshCcw className="icon"/><span>New Chat</span></button>
    </div>
  );
}
  ...
  return (
    <div className="weather-card glass rounded-xl p-3">
      <div className="flex items-center gap-3">
        {card.icon && (
          <img src={card.icon} alt="" className="w-12 h-12 rounded-md bg-white/5 border border-white/10 object-contain" loading="lazy" referrerPolicy="no-referrer" />
        )}
        <div className="min-w-0">
          <div className="text-sm font-semibold truncate">{card.title || "Weather"}</div>
          <div className="text-xs small-label truncate">{card.subtitle || ""}</div>
        </div>
      </div>
      ...
      {hrs.length > 0 && (
        <div className="mt-3">
          <div className="text-[11px] small-label mb-1">Next hours</div>
          <div className="w-hscroll flex gap-8 overflow-x-auto -mx-1 px-1 pb-1">
            {hrs.map((h, i) => (
              <div key={i} className="w-hour glass rounded-lg p-2 min-w-[86px] text-center">
                <div className="text-[11px] small-label">{h.t ? hourLabel(h.t) : (h.text || "").split(" ")[0]}</div>
      ...
      {days.length > 0 && (
        <div className="mt-3">
          <div className="text-[11px] small-label mb-1">Next days</div>
          <div className="grid grid-cols-3 gap-2">
            {days.map((d, i) => (
              <div key={i} className="glass rounded-lg p-2 text-center">
                <div className="text-[11px] small-label truncate">{d.day || `Day ${i + 1}`}</div>
                ...
                {d.text && <div className="text-[11px] small-label mt-1 line-clamp-2">{d.text}</div>}
              </div>
            ))}
          </div>
        </div>
...
function AIChat() {
  ...
  // keep viewport safe with keyboard
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    previewTimer.current = setTimeout(async () => {
      try {
        ...
        const newsRanked = rankAndTrim((rn?.data?.cards || []).filter(Boolean).map(c => ({ ...c, image: firstImageUrl(c) || c.image, type: c.type || "news" })), 10, true).filter(c => !!bestPreview(c, true));
  ...
  const fetchFollowups = async (q) => {
    try { const { data } = await axios.get(`${API_BASE}/suggest`, { params: { q, mode: "followup" } });
  ...
  const pushWithFollowups = async (md, cards, q, meta={}) => {
    ...
    if (speakOn && md) { try { window.speechSynthesis.cancel(); const u=new SpeechSynthesisUtterance(md.replace(/[#*_`>]/g," ")); speechSynthesis.speak(u);} catch {} }
  };
    ...
    try {
      ...
      if (lower.startsWith("google:")) {
        ...
        try {
          ...
          if (!hasMedia && (wantsImages(q) || isSearchy(q))) { const imgs = await axios.post(`${API_BASE}/realtime`, { query: q, intent: "images" }); const g=(imgs?.data?.cards||[]).find(c=>c.type==="images"); if(g) cards = cards.concat([g]); }
      ...
      if (lower.startsWith("search:")) {
        ...
        try {
          ...
          let cards = rankAndTrim((r.data?.results || []).filter(Boolean).map(it => ({ type:"web", title: it.title, url: it.url, image: it.image || null, source: it.source, snippet: it.snippet })), 12, true);
      ...
      if (wantsNews(content)) {
        ...
        try {
          ...
          cards = rankAndTrim((r.data?.cards || []).filter(Boolean).map(c => ({ ...c, image: firstImageUrl(c) || c.image, type: c.type || "news" })), 12, true).filter(c => !!bestPreview(c, true));
        } catch {}
      ...
      if (wantsCrypto(content)) {
        ...
        await pushWithFollowups(r.data?.markdown || "Crypto:", (r.data?.cards || []).filter(Boolean), content);
        return;
      }
  ...
  /* render helpers (SmartImage/LinkPreview/MediaBlock, OrganizedAnswer)… kept as in your last version */
  const SmartImage = ({ url, title }) => {
    ...
    const onErr=(e)=>{ const el=e.currentTarget; const step=el.dataset.step||"orig"; if(step==="orig"){ el.dataset.step="proxy"; el.src=toProxy(url); return; } if(step==="proxy"){ el.dataset.step="fallback"; el.src=unsplash(title||"image")||""; return; } el.style.display="none"; };
    return <img ref={elRef} alt="" className="w-full rounded-lg glass" loading="lazy" referrerPolicy="no-referrer" onError={onErr} onLoad={(e)=>{e.currentTarget.style.opacity=1;}} style={{opacity:0,transition:"opacity .2s ease"}}/>;
  };
    ...
    return (
      <a href={card.url} target="_blank" rel="noreferrer" className="favicon-only">
        ...
        <div className="min-w-0">
          ...
          <div className="small-label truncate">{displaySource(card)}</div>
        </div>
      </a>
    );
  };
                ...
                {items.map((it, j) => {
                  ...
                  return (
                    <a key={j} href={href} target="_blank" rel="noreferrer" className="block">
                      ...
                      {it.source && <div className="mt-1 text-[10px] small-label">{it.source}</div>}
                    </a>
                  );
                })}
              </div>
          ...
          if (card.type === "youtube" || isYouTube(card.url || "")) {
            ...
            return (
              <div key={i} className="embed-responsive embed-16by9 rounded overflow-hidden glass" style={{ maxHeight: 280 }}>
                <iframe src={`https://www.youtube.com/embed/${id}`} title={card.title || "YouTube"} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen />
              </div>
            );
          }
          return null;
  ...
  const buildLinkSets = (cards = [], searchy = false) => {
    const links = (cards || []).filter(c => ["web","link","wiki","news","stock","crypto","images"].includes(c.type) && c.url && !isFilteredSource(c.url));
    ...
    const quickActions = [];
    for (const ph of pref) { const k = Object.keys(byHost).find(h => h===ph || h.endsWith("."+ph)); if (k) quickActions.push(byHost[k]); }
    const grid = Object.values(byHost).filter(c => !/^(google\.com|wikipedia\.org)$/.test(host(c.url)) || searchy).sort((a,b)=> scoreCard(b)-scoreCard(a)).slice(0,6);
  ...
  const OrganizedAnswer = ({ md }) => {
    ...
    return (
      <>
        ...
        {pairs.length>0 && (<div className="org-section"><div className="org-sub">Quick Chart</div><div className="bars">{pairs.map((p, i) => { const max = Math.max(...pairs.map(x=>Math.abs(x.value)), 1); const w = Math.round(Math.min(100, (Math.abs(p.value)/max)*100)); return (<div key={i} className="bar-row"><div className="bar-label">{p.label}</div><div className="bar-track"><div className="bar-fill" style={{ width: `${w}%` }} /></div><div className="bar-val">{p.value}</div></div>); })}</div></div>)}
        <div className="org-section"><div className="org-sub">Full answer</div>
          <div className="answer expanded">
            <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]}
              components={{
                ...
                a: ({node, ...props}) => <a {...props} className="underline decoration-gray-600" target="_blank" rel="noreferrer" />,
  ...
  const sendImageForAnalysis = async (file) => {
    ...
    } catch { await pushWithFollowups("Image analysis failed.", [], "vision failed", { suppressSources: true }); }
  };
  const sendAnyFile = async (file) => {
    ...
    } catch { await pushWithFollowups("File analysis failed.", [], "file failed", { suppressSources: true }); }
  };
  const runDeepResearch = async () => {
    const q = (input || "").trim(); if (!q) return; setInput("");
    try { const r = await axios.post(`${API_BASE}/deepsearch`, { q, agent: agentOn }); await pushWithFollowups(r.data?.answer || `Deep research on **${q}**`, r.data?.cards || [], q); }
    catch { await pushWithFollowups("Deep research failed.", [], q, { suppressSources: true }); }
  };
    ...
    <div className={`flex flex-col min-h-[100svh] ${codeMode ? "code-mode" : ""}`}>
      ...
      <header>
        <div className="header-row">
          <div className="brand">
            <span>Droxion</span>
            <span className="small-label">• Lite</span>
          </div>

          {/* Left group sits under brand on small screens */}
          <div className="left-tools">
            <button onClick={()=>setTheme(t=> t==="dark"?"light":"dark")} className="pill-btn" title="Toggle theme">
              {theme==="dark" ? <FiMoon /> : <FiSun />} <span>{theme==="dark"?"Dark":"Light"}</span>
            </button>
            ...
            <button onClick={()=>setProMode(v=>!v)} className="pill-btn" title="Premium">
              <FiStar /><span>Pro {proMode?"On":"Off"}</span>
            </button>
            <button onClick={()=>setSpeakOn(v=>!v)} className="pill-btn" title="Speak replies">
              {speakOn ? <FiVolume2 /> : <FiVolumeX />}<span>{speakOn?"On":"Off"}</span>
            </button>
            <button onClick={()=>{/* your mic starter here if you add it */}} className="pill-btn" title="Speak your message">
              <FiMic /><span>Mic</span>
            </button>
          </div>
          ...
          <div className="right-tools">
            ...
            <div style={{ position:"relative" }}>
              ...
              {menuOpen && (
                <div className="absolute right-0 top-[110%]" onMouseLeave={()=>setMenuOpen(false)}>
                  <ToolsMenu
                    ...
                    onClearAll={()=>{ setMessages([]); localStorage.removeItem(STORAGE_KEY); localStorage.removeItem(MEM_KEY); setMenuOpen(false); }}
                    onNewChat={()=>{ setMessages([]); localStorage.removeItem(STORAGE_KEY); setInput(""); window.scrollTo({ top: 0, behavior: "smooth" }); setMenuOpen(false); }}
                  />
                </div>
              )}
            </div>
        ...
        <div className="max-w-4xl mx-auto w-full px-3 pb-32 pt-3">
          <div className="space-y-4">
            {messages.map((msg, i) => {
              ...
              const shouldShowSources = (!isUser) && (showSearchy || isRealtimeCard) && hasPreviewable && !(msg.meta && msg.meta.suppressSources);
              ...
              return (
                <div key={i} className={`msg ${isUser ? "glass-2" : "glass"}`}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {!isUser && msg.content && (
                      <button onClick={()=>copyMessage(i)} className="text-xs small-label inline-flex items-center gap-1" title="Copy">
                  ...
                  {!isUser && shouldShowSources && (
                    <>
                      <div className="actions-row" style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:8}}>
                        <a href={googleUrl} target="_blank" rel="noreferrer" className="pill-btn">google: {userPrompt || "search"}</a>
                        <a href={newsUrl} target="_blank" rel="noreferrer" className="pill-btn">search: {userPrompt ? `${userPrompt} latest news` : "latest news"}</a>
                        {linkSets.quickActions.slice(0,2).map((c,idx)=>(
                          <a key={`qa-${idx}`} href={c.url} target="_blank" rel="noreferrer" className="pill-btn">
                  ...
                  {!isUser && Array.isArray(msg.followups) && msg.followups.length>0 && (
                    <div className="mt-3 flex flex-wrap gap-8">
                      {msg.followups.slice(0,3).map((s,idx)=>(
                        <button key={idx} onClick={()=>handleSend(s)} className="pill-btn">
                          {s}
                        </button>
                      ))}
                    </div>
            ...
            {typing && (
              <div className="glass rounded-xl p-4">
                <div className="h-4 w-24 skel mb-2 rounded" />
                <div className="h-3 w-full skel mb-1 rounded" />
                <div className="h-3 w-4/5 skel mb-1 rounded" />
                <div className="h-3 w-3/5 skel rounded" />
              </div>
            )}
          </div>
        </div>
        ...
        <div className={`fixed-preview fixed-panel ${input.length ? "dim-while-typing" : ""}`}>
          <div className="max-w-4xl mx-auto px-3">
            <div className="panel glass rounded-xl p-2 suggestions-panel">
              {/* headlines + weather + crypto + suggestions kept like before */}
              {/* (unchanged from your last version to keep this message short) */}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="fixed-bottom" style={{ paddingBottom:"max(env(safe-area-inset-bottom), 12px)" }}>
        <div className="max-w-4xl mx-auto px-3 pt-2 pb-2">
          <div className="flex items-center gap-2">
            <div className="flex-1 rounded-2xl" style={{ border:"1px solid var(--border)", background:"var(--panel)", padding:"6px 10px" }}>
              <textarea
                ref={inputRef}
                value={input}
            ...
            <button onClick={()=>handleSend(input)} className="pill-btn" title="Send">
              <FiArrowRight />
            </button>
          </div>
          ...
          <div className="flex gap-6 flex-wrap mt-2">
            {["Cinematic","Anime","Futuristic","Fantasy","Realistic"].map((s)=>(
              <button key={s} onClick={()=>handleSend(`steps to do ${s.toLowerCase()} project`)} className="pill-btn">{s}</button>
            ))}
          </div>
        </div>
      </div>
