...
function Rankings() {
  return (
    <section className="lfPage lfPlaceholder">
      <div className="lfEyebrow"><Trophy size={16} /> DROXION RANKINGS</div>
      <h1>Top creators will live here.</h1>
      <p>Daily, weekly, monthly, rising creators, most viewed and most gifted rankings are next after the LIVE core is stable.</p>
      <div className="lfComingCard">
        <Crown size={28} />
        <div><strong>Rankings foundation ready</strong><span>We will connect this to real LIVE views, follows and gifts.</span></div>
      </div>
    </section>
  );
}
...
function Inbox() {
  return (
    <section className="lfPage lfPlaceholder">
      <div className="lfEyebrow"><Bell size={16} /> INBOX</div>
      <h1>LIVE activity and messages.</h1>
      <p>Creator-goes-live alerts, guest requests, follows, gifts and direct messages will be consolidated here.</p>
    </section>
  );
}
  ...
  return (
    <main className="lfShell">
      <header className="lfTopbar">
        ...
        <button className="lfCoins" type="button" onClick={() => setWalletOpen(true)}>
          <Coins size={17} /> {coins}
        </button>
      </header>

      <div className="lfContent">{content}</div>
      ...
      <nav className="lfNav" aria-label="Droxion navigation">
        {TABS.map(item => {
          ...
          return (
            ...
            >
              <span className="lfNavIcon"><Icon size={item.primary ? 23 : 20} /></span>
      ...
      {walletOpen && (
        <DroxionWallet
          ...
          onBalanceRefresh={() => refreshWallet()}
        />
      )}
    </main>
  );
