...
export async function getSupabaseUser(accessToken) {
  ...
  // Supabase Auth GET /user returns the user object directly. Keep the
  // payload.user fallback for compatibility with wrapped responses.
...
export async function findTransactionByOrderId(_accessToken, orderId, userId) {
  ...
  if (!response.ok) {
    const detail = Array.isArray(data) ? '' : data?.message || data?.error;
    throw new Error(detail || 'Unable to look up the purchase record.');
  }
