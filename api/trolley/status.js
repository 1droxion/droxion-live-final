import { findTrolleyRecipient, getSupabaseUser, safePayoutProfile, syncTrolleyProfile } from './lib.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const ready = Boolean(
      ...
      profile?.payoutMethod === 'bank-transfer' &&
      profile?.country &&
      profile?.currency
    );
    ...
    res.status(200).json({
      ...
      recipientStatus: profile?.status || null,
      complianceStatus: profile?.complianceStatus || null,
      payoutMethod: profile?.payoutMethod || null,
      routeMinimum: profile?.routeMinimum ?? null,
      estimatedFee: profile?.estimatedFee ?? null,
