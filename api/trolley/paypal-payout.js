import { callRpc, getSupabaseConfig, getSupabaseHeaders, getSupabaseUser, trolleyRequest } from '../../server/trolley-lib.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const creatorCoins = Math.floor(Number(req.body?.creatorCoins || 0));
    ...
    if (!recipientId) throw new Error('Verified Trolley PayPal recipient was not found.');

    const batchPayload = await trolleyRequest('/v1/batches', {
      method: 'POST',
      body: { currency: 'USD', description: `Droxion PayPal creator payout ${requestId}`, tags: ['droxion', 'paypal', requestId] }
    });
    ...
    res.status(error?.status && Number.isInteger(error.status) ? error.status : 400).json({
      error: error?.message || 'Could not send PayPal payout through Trolley.'
    });
  }
}
