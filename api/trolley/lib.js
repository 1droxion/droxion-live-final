...
export function getTrolleyConfig() {
  ...
  if (!accessKey || !secretKey) {
    throw new Error('Trolley payout credentials are not configured.');
  }
...
export async function findTrolleyRecipient(referenceId) {
  ...
  const recipients = payload?.recipients || payload?.recipient ? (payload.recipients || [payload.recipient]) : [];
...
export function safePayoutProfile(recipient) {
  ...
  return {
    ...
    status: String(recipient.status || '').toLowerCase(),
...
export async function syncTrolleyProfile(userId, recipient) {
  ...
  await callRpc(null, 'droxion_sync_trolley_payout_profile', {
    p_user_id: userId,
    p_provider_recipient_id: profile.recipientId,
    p_country_code: profile.country || null,
    p_currency: profile.currency || null,
    p_recipient_status: profile.status || null,
    p_compliance_status: profile.complianceStatus || null,
    p_payout_method: profile.payoutMethod || null,
    p_route_type: profile.routeType || null,
    p_route_minimum: profile.routeMinimum,
    p_estimated_fee: profile.estimatedFee,
    p_bank_name_masked: profile.bankName,
    p_account_last4: profile.accountLast4
  });
  return profile;
}
...
export function extractLocalQuote(payment, fallbackCurrency = '') {
  ...
  const destinationAmount = Number(
    payment.targetAmount ?? payment.destinationAmount ?? payment.recipientAmount ?? payment.amount ?? 0
  );
  const destinationCurrency = String(
    payment.targetCurrency || payment.destinationCurrency || payment.recipientCurrency || fallbackCurrency || payment.currency || ''
  ).toUpperCase();
